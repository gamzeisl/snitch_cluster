...
void sc_st_mxgemm(mxgemm_args_t* gemm_args, void* a, void* b, uint32_t beta,
    ...
    precision_t prec_dst = gemm_args->prec_dst;
    ...
    if (snrt_is_compute_core()) {
        ...
        if (frac_m > 0)
            impl(frac_m, n, k, a + offsetA, lda_strided, transa, b, ldb, transb,
                 c + offsetC, ldc_strided, (float)beta, setup_ssr, dual_ssr,
                 scales + offsetS, lds_strided, block_size, scales_a+offsetS_ab, scales_b, ldsab_strided, 
                 sub_byte);
    }
}
...
int mxgemm(mxgemm_args_t* args) {
    mxgemm_args_t* local_args = snrt_l1_next();
    ...
    precision_t prec_dst = (precision_t)local_args->prec_dst;
    ...
    void* heap_ptr = (void*)local_args + sizeof(mxgemm_args_t);
    if (load_a) {
        local_a = heap_ptr;
